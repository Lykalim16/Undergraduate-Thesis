/* Combination of Lockdown and Quarantine */
...
global {
	...
	file buildings_shapefile <- file("../includes/NonQ_building.shp");
	file qbuildings_shapefile <- file("../includes/Q_building.shp");
	...
	bool lockdown <- true update: (current_day > 6 and current_day < 21) or (current_day > 36 and current_day < 51) or (current_day > 66 and current_day < 81) or (current_day > 96 and current_day < 111) or (current_day > 126 and current_day < 141) or (current_day > 156 and current_day < 171); /* lockdown every 2nd and 3rd week of the month */
	...
	init {
		...
		create qbuilding from: qbuildings_shapefile;
...
species people skills: [moving] {
	...
	reflex lockdown when: lockdown {
		target <- nil;
		staying_counter <- 0;
		if flip(staying_counter / staying_coeff) { /* If mababa percentage, no new target */
	...
	reflex stay when: target = nil {
		if(not lockdown){
			...
			if flip(staying_counter / staying_coeff) {
				if(is_infected and (cycle >= 1728)){ /* If infected and after day 07 (bc day 07 pa iimplement yung isolation) */
					if((cycle-time_infected >= 288) and (cycle-time_infected <= 4320)){ /* Quarantine for 14 days or until recovered */
						target <- any_location_in (one_of(qbuilding));
					}else{
						target <- any_location_in(one_of(building));
					}
				}else{
			...
			}else if (is_recovered or is_dead){
				target <- any_location_in(one_of(building));
			}
		}
	}
...
species building {
	...
	reflex let_people_leave {
		...
		release people_in_building where (flip(each.staying_counter / staying_coeff)) as: people in: world {
			if(is_infected and (cycle >= 2016)){ /* In infected and after day 07 (bc day 07 pa iimplement yung isolation) */
				if((cycle-time_infected >= 288) and (cycle-time_infected <= 4320)){ /* Quarantine for 14 days or until recovered */
					target <- any_location_in (one_of(qbuilding));
				}else{
					target <- any_location_in(one_of(building));
...
experiment main_experiment type: gui { /* experiment with a graphical interface */
	...
	output {
		...
		display view3D type: opengl antialias: false {
			...
			species qbuilding transparency: 0.3;
		}
